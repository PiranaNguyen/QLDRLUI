...
const AdminEvaluationForm = () => {
    ...
    const handleViewDetails = (evaluation) => {
        axios.get(`http://localhost:8080/api/eva-details/eva/${evaluation.id}`)
            .then(response => {
                ...
                setIsEditing(false); // Đảm bảo rằng không cho phép chỉnh sửa
                setShowModal(true);
            })
            .catch(error => {
                setError(error);
    ...
    const handleSave = () => {
        ...
        const request = axios.post('http://localhost:8080/api/evaluate', newEvaluation);
        request
            .then(() => {
...
<Modal show={showModal} onHide={() => setShowModal(false)}>
                <Modal.Header closeButton>
                    <Modal.Title>{isEditing ? 'Thêm Đánh Giá' : 'Xem Chi Tiết'}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form>
                        ...
                        <Form.Group controlId="formStudent">
                            ...
                            <Form.Control
                                type="text"
                                name="clazz"
                                value={currentEvaluation.clazz = cls}
                                onChange={handleChange}
                                disabled
                            />
                        </Form.Group>
        ...
        <React.Fragment key={type.id}>
            ...
            {subCriteriaTypes
                ...
                <Modal.Footer>
                    ...
                    {isEditing && (<Button variant="primary" onClick={handleSave}>Lưu Đánh Giá</Button>)}
                </Modal.Footer>
            </Modal>
        </div>
    );
